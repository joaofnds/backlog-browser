...
describe("parseOptions", () => {
  ...
  test("reads the idle timeout as minutes", () => {
    expect(parseOptions(["--idle-timeout", "2"]).idleTimeoutMs).toEqual(120_000);
  });
  ...
  test("accepts flags before the root", () => {
    expect(parseOptions(["--port", "7000", "/tmp"])).toMatchObject({ port: 7000, root: "/tmp" });
  });
  ...
  describe("when a numeric flag is out of range", () => {
    ...
    test("rejects a negative idle timeout", () => {
      expect(() => parseOptions(["--idle-timeout", "-1"])).toThrow(/--idle-timeout/);
    });
  });
