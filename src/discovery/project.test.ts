...
describe("Project", () => {
  describe("slug", () => {
    ...
    test("is stable for the same path", () => {
      expect(projectAt("/code/alpha").slug).toEqual(projectAt("/code/alpha").slug);
    });
  ...
  describe("byName", () => {
    test("orders case-insensitively", () => {
      const sorted = [projectAt("/a", "banana"), projectAt("/b", "Apple")].sort(Project.byName);

      expect(sorted.map((project) => project.name)).toEqual(["Apple", "banana"]);
    });
  });
});
